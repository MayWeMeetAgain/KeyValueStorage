...
public class DatabaseServer {
    ...
    public CompletableFuture<DatabaseCommandResult> executeNextCommand(RespArray message) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            int commandNamePos = DatabaseCommandArgPositions.COMMAND_NAME.getPositionIndex();
            
            return DatabaseCommands.valueOf(objects.get(commandNamePos).asString()).getCommand(env, objects).execute();
        }, executorService);
    }
