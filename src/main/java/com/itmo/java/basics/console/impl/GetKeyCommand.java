...
public class GetKeyCommand implements DatabaseCommand {
    ...
    public GetKeyCommand(ExecutionEnvironment env, List<RespObject> commandArgs) {
        if (commandArgs == null) {
            throw new NullPointerException("Arguments list for GetKey command is null");
        }
        if (commandArgs.size() != NUM_OF_ARG) {
            throw new IllegalArgumentException(String.format("Wrong number of arguments for GetKey, expected: %d, given: %d", NUM_OF_ARG, commandArgs.size()));
        }
    ...
    public DatabaseCommandResult execute() {
        ...
        try {
            value = env.getDatabase(dbName).get().read(tableName, key).get();
        } catch (DatabaseException e) {
            return DatabaseCommandResult.error(String.format("Reading from storage cell with parameters: %s, %s, %s failed", dbName, tableName, key));
